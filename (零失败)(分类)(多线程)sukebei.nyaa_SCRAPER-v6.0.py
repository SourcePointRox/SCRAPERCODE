    ...
    if not size:
        try:
            ...
            if body:
                ...
                # 优先匹配带标签的 size
                size_match = re.search(
                    r"(?:total\s*size|file\s*size)[:\s]*((?:\d+\.?\d*)\s*(?:Bytes?|KB|MB|GB|TB|PiB))",
                    body_text, re.IGNORECASE
                )
                if size_match:
                    size = size_match.group(1).strip()
                else:
                    all_sizes = re.findall(
                        r"((?:\d+\.?\d*)\s*(?:Bytes?|KB|MB|GB|TB|PiB))",
                        body_text, re.IGNORECASE
                    )
                    if all_sizes:
                        ...
                        for s in reversed(all_sizes):
                            if re.search(r"(PiB|TB)", s, re.IGNORECASE):
                                size = s.strip()
                                break
                        if not size:
                            for s in reversed(all_sizes):
                                if re.search(r"GB", s, re.IGNORECASE):
                                    size = s.strip()
                                    break
                        if not size:
...
def worker_scrape_one(session_manager: IsolatedSessionManager, base_url: str, target_id: int,
                      ...
                      circuit_breaker: CircuitBreaker, written_ids: Set[int]) -> Tuple[bool, str]:
    """
...
def retry_worker(session_manager, base_url, state, rate_limiter, result_queue,
                 ...
                 retry_ids, success_counter, written_ids):
    """重试工作线程"""
        ...
        for chunk in chunks[:num_threads]:
            ...
            t = threading.Thread(
                ...
                args=(session_manager, base_url, state, rate_limiter, result_queue,
                      failed_queue, connect_timeout, read_timeout, retry_count,
                      chunk, success_counter, written_ids),
...
def scrape(proxies, params):
    ...
    writer = threading.Thread(
        ...
        args=(out_file, fieldnames, result_queue, state, written_ids),
        name="WriterThread",
