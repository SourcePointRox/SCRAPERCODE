#!/usr/bin/env python3
"""
sukebei.nyaa.si 全量元数据爬虫  v2.1 直链遍历版（增强断点续爬）
...
def print_banner():
    print(c("""
╔══════════════════════════════════════════════════════════════╗
║  sukebei.nyaa.si 全量元数据爬虫  v2.1 直链遍历版（增强断点）  ║
...
def setup_params():
    """
    配置爬取参数。启动时自动扫描目录下 sukebei_nyaa_data_*.csv 文件，
    检测到已有文件时提供一键续爬选项。
    """
    DEFAULT_START_ID = 92
...
def load_existing_ids(out_file):
    """从CSV中读取已爬取的detail_url和id列，提取ID集合用于跳过"""
    ids = set()
...
def scrape(proxies, params):
    ...
    with open(out_file, "a", newline="", encoding="utf-8-sig") as f:
        ...
        while current_id <= end_id:
            ...
            eta_str = f"{int(eta_seconds // 3600)}h{int((eta_seconds % 3600) // 60)}m" if eta_seconds < 86400 else f"{int(eta_seconds // 86400) // 3600}h"
