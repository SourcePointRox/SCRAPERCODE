#!/usr/bin/env python3
"""
sukebei.nyaa.mom 全量元数据爬虫  v7.0 — 真正零失败 + 极限效率版
...
# 状态码分类
# v7.0: 3xx 重定向不应视为不存在, sukebei.nyaa.mom 对不存在的ID可能返回302重定向到首页
# 需要检查 Location header 判断是否真的重定向走了
...
def safe_input(prompt, default=""):
    """
    Windows 控制台编码兼容的 input()。
    避免 UnicodeDecodeError / UnicodeEncodeError (常见于 Windows GBK/UTF-16 终端)。
    """
    try:
        return input(prompt).strip()
    except (UnicodeDecodeError, UnicodeEncodeError):
        # Windows 终端编码不兼容时，尝试用 ASCII 方式重新提示
        try:
            # 重置终端编码后重试
...
def print_banner():
    print(c("""
╔══════════════════════════════════════════════════════════════════╗
║        sukebei.nyaa.mom 全量元数据爬虫  v7.0 真正零失败版        ║
...
class IsolatedSessionManager:
    ...
    def _create_session(self) -> requests.Session:
        ...
        session.headers.update({
            ...
            # 注意: 不要加 "br" (Brotli)！Python requests 默认不带 brotli 库，
            # 如果服务端返回 Content-Encoding: br，iter_content 解压会失败，
            # 导致只收到 ~17KB 压缩乱码而非 150KB+ 正常页面
...
def request_with_retry(session, url, connect_timeout=15, read_timeout=60, retry_count=10,
    ...
    for attempt in range(retry_count + 1):
        try:
            ...
            # 流式读取 — v7.1: 使用 bytearray + 流式异常不再静默吞掉
            buf = bytearray()
            stream_error = None
            try:
                ...
                stream_error = e
                log_error(f"流式读取异常 (URL: {url[:120]}): {type(e).__name__}: {e}", e)
            finally:
                try:
                    r.close()
                except Exception:
            ...
            # 确保 encoding 被正确设置，避免 r.text 解码失败
            ...
            # ── 重定向 (3xx) — 需要区分是内容重定向还是"不存在"重定向 ──
            if r.status_code in REDIRECT_STATUS:
                location = r.headers.get("Location", "")
                # 如果重定向到首页或非 /view/ 页面, 说明该ID不存在
                if location and "/view/" not in location:
                    return r, FailureClassifier.NOT_FOUND
                # 否则视为可重试 (可能是临时重定向)
                return r, FailureClassifier.RETRYABLE
...
def parse_detail_page(html_text: str, detail_url: str) -> Tuple[str, ...]:
    ...
    # ── 面板数据提取 ──
    # 注意: sukebei.nyaa.mom 同样使用 IEC 二进制单位 (GiB, MiB, KiB, TiB)
    ...
    try:
        ...
        for row_div in rows_divs:
            try:
                ...
                if not category:
                    # v7.0: 兼容新旧两种 HTML 结构
                    # 旧结构: Category 和值在不同的子 div/元素中
                    # 新结构: "Category: Real Life - Videos" 混在同一行文本中
                    # 策略 1: 从合并文本中正则提取
                    cat_match = re.search(r"Category[:\s]+(.*?)(?:\s+Date|\s*$)", text, re.IGNORECASE)
                    if cat_match:
                    ...
                    if not category:
                        for child in row_div.children:
                            ...
                            if any(k in child_text for k in ["Category", "カテゴリ", "分类"]):
                                ...
                                break
                    # 策略 3: 从 category 链接提取
                    if not category:
                        cat_links = soup.select('a[href*="?c="]')
                        if cat_links:
                            parts = [link.get_text(strip=True) for link in cat_links if link.get_text(strip=True)]
                            category = " - ".join(parts)
            except Exception:
                continue
    except Exception:
...
def detect_latest_id(session_manager: IsolatedSessionManager, base_url: str,
    ...
    try:
        ...
        for page in range(1, 4):
            ...
            # 使用官方排序参数 (兼容新旧格式)
            page_url = f"{base_url}/?sort=id&order=desc&page={page}" if page > 1 else f"{base_url}?sort=id&order=desc"
            ...
            if r and r.status_code == 200:
                try:
                    ...
                    # Cloudflare 拦截检测 — 多种特征匹配
                    cf_markers = ['challenge-platform', 'cf-browser-verification',
                                  'Just a moment', 'cf-challenge', '_cf_chl',
                                  'cf-disturbed', 'cloudflare']
                    cf_detected = any(mk in html_text.lower() for mk in cf_markers)

                    # 判断是否为异常短页面 (< 30KB 大概率是拦截页/错误页，正常首页 > 100KB)
                    is_abnormally_short = html_len < 30000

                    if cf_detected or is_abnormally_short:
                        reason = "Cloudflare 拦截" if cf_detected else f"页面异常短 ({html_len} bytes，正常应 > 100KB)"
                        print(c(f"  ⚠ 疑似被 {reason}，跳过首页解析 (page={page})", "yellow"), flush=True)
                        if cf_detected:
                            # 保存 CF 页面前500字节到日志，方便排查
                            log_error(f"首页被 Cloudflare 拦截 (page={page}, HTML长度={html_len})", None)
                        else:
                            log_error(f"首页内容异常短 (page={page}, HTML长度={html_len})", None)
                        break
                    ...
                    if not rows:
                        # 输出诊断信息
                        print(c(f"  ⚠ 首页未找到 torrent-list 表格行 (page={page})", "yellow"), flush=True)
                        print(c(f"    HTML长度: {html_len} bytes", "dim"), flush=True)
                        # 检查有哪些 table
                        tables = soup.find_all("table")
                        print(c(f"    页面中 table 数量: {len(tables)}", "dim"), flush=True)
                        for ti, t in enumerate(tables):
                            cls = t.get("class", [])
                            print(c(f"    table[{ti}] class={cls}", "dim"), flush=True)
                        break
                    page_ids_count = 0
                    for row in rows:
                        ...
                        for link in links:
                            ...
                            if m:
                                ...
                                page_ids_count += 1
                    if page_ids_count > 0:
                        print(c(f"    第 {page} 页: 采集到 {page_ids_count} 个ID", "dim"), flush=True)
                except Exception as parse_err:
                    log_error(f"首页解析异常 (page={page})", parse_err)
                    pass
            else:
                status = r.status_code if r else 'None'
                print(c(f"  ⚠ 首页请求失败: HTTP {status} (page={page})", "yellow"), flush=True)
                break  # 第一页就失败则不继续
    ...
    try:
        ...
        if r and r.status_code == 200:
            try:
                rss_text = r.text
                rss_len = len(rss_text)

                # RSS 也检测 CF 拦截
                if any(mk in rss_text.lower() for mk in ['challenge-platform', 'cf-browser-verification', 'Just a moment', '_cf_chl']):
                    print(c(f"  ⚠ RSS 也被 Cloudflare 拦截, 长度: {rss_len}", "yellow"), flush=True)
                else:
                    # RSS 中 /view/ 链接可能在 <guid> 或 CDATA 中
                    ids = re.findall(r"/view/(\d+)", rss_text)
                    if ids:
                        max_id = max(int(x) for x in ids)
                        print(c(f"✔ 检测到最新条目ID: {max_id} (RSS, 共 {len(ids)} 个)", "green"))
                        return max_id
                    else:
                        print(c(f"  ⚠ RSS 返回内容但未找到 /view/ 链接, 内容长度: {rss_len}", "yellow"), flush=True)
                        # 输出 RSS 前200字节帮助诊断
                        log_error(f"RSS 无 /view/ 链接 (长度={rss_len}), 前200字节: {rss_text[:200]}", None)
            except Exception as rss_err:
                log_error("RSS 解析异常", rss_err)
    except Exception as rss_ex:
        log_error("RSS 请求失败", rss_ex)
    ...
    try:
        ...
        for tv in test_values:
            ...
            if r and r.status_code == 200:
                # 检查返回的是否是 CF 拦截页（CF 页面也返回 200）
                body = r.text
                if any(mk in body.lower() for mk in ['challenge-platform', 'cf-browser-verification',
                                                        'Just a moment', '_cf_chl', 'cf-disturbed']):
                    cf_blocked = True
                    print(c(f"  ✗ /view/{tv} 也被 Cloudflare 拦截", "red"), flush=True)
                    break
                if len(body) < 5000:
                    # 异常短的页面，可能也是拦截
                    cf_blocked = True
                    print(c(f"  ✗ /view/{tv} 返回异常短内容 ({len(body)} bytes)，疑似拦截", "red"), flush=True)
                    break
                latest = tv
                lower = tv
                break
        ...
        if not cf_blocked:
            ...
            print(c("", "reset"))
            print(c("  ╔══════════════════════════════════════════════════╗", "red"))
            print(c("  ║  ⛔  所有检测方式均被 Cloudflare 拦截！           ║", "red"))
            print(c("  ║                                                  ║", "red"))
            print(c("  ║  原因: 你的网络 IP 被 Cloudflare 识别为机器人    ║", "yellow"))
            print(c("  ║                                                  ║", "yellow"))
            print(c("  ║  解决方案:                                        ║", "green"))
            print(c("  ║  1. 使用 HTTP/SOCKS5 代理 (推荐)                 ║", "green"))
            print(c("  ║  2. 换一个网络/IP (如手机热点)                   ║", "green"))
            print(c("  ║  3. 使用 VPN                                      ║", "green"))
            print(c("  ╚══════════════════════════════════════════════════╝", "red"))
            print(c("", "reset"))
            log_error("Cloudflare 拦截: 首页、RSS、二分探测全部失败，需要代理", None)
    except Exception as e:
        log_error("二分探测失败", e)
        print(c(f"✗ 自动检测失败: {e}", "red"))

    # Windows 控制台编码兼容 — 使用 safe_input 避免 UnicodeDecodeError
    default_id = 4559759
    manual = safe_input(c(f"请手动输入最新条目ID [默认 {default_id}]: ", "yellow"))
    return int(manual) if manual.isdigit() else default_id
...
def setup_resume(candidates, default_start_id):
    ...
    print()
    choice = safe_input(c("请选择 [默认 1 续爬 / 0 新建]: ", "yellow"))
    if choice == "0":
...
def setup_proxy():
    ...
    print("  3. SOCKS5 代理")
    choice = safe_input(c("请选择 [1/2/3]: ", "yellow"))
    proxies = None
    if choice == "2":
        host = safe_input("HTTP 代理主机 (如 127.0.0.1): ")
        port = safe_input("HTTP 代理端口 (如 7890): ")
        if host and port:
        ...
        host = safe_input("SOCKS5 代理主机 (如 127.0.0.1): ")
        port = safe_input("SOCKS5 代理端口 (如 1080): ")
        user = safe_input("用户名 (无则回车): ")
        pwd = safe_input("密码   (无则回车): ")
        if host and port:
            if user and pwd:
...
def setup_params():
    ...
    if resume_file:
        start_id_input = safe_input(c(f"起始条目ID [默认 {resume_start}，从断点继续]: ", "yellow"))
        start_id = int(start_id_input) if start_id_input.isdigit() else resume_start
        out_file_input = safe_input(c(f"输出文件名 [默认 {resume_file}，续爬合并]: ", "yellow"))
        out_file = out_file_input if out_file_input else resume_file
    else:
        start_id_input = safe_input(c(f"起始条目ID [默认 {DEFAULT_START_ID}]: ", "yellow"))
        start_id = int(start_id_input) if start_id_input.isdigit() else DEFAULT_START_ID
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        default_out = f"{CSV_PREFIX}_{ts}.csv"
        out_file = safe_input(c(f"输出文件名 [默认 {default_out}]: ", "yellow")) or default_out

    end_id_input = safe_input(c("结束条目ID [默认 auto=自动检测最新]: ", "yellow")).lower()
    end_id = "auto"
    if end_id_input and end_id_input != "auto":
        end_id = int(end_id_input) if end_id_input.isdigit() else "auto"

    threads_input = safe_input(c("并行线程数 [默认 16]: ", "yellow"))
    num_threads = int(threads_input) if threads_input.isdigit() and int(threads_input) > 0 else 16

    qps_input = safe_input(c("全局最大请求速率 QPS [默认 20]: ", "yellow"))
    max_qps = float(qps_input) if qps_input.strip() else 20.0

    connect_timeout_input = safe_input(c("连接超时时间（秒）[默认 10]: ", "yellow"))
    try:
        connect_timeout = float(connect_timeout_input)
    except Exception:
        connect_timeout = 10

    read_timeout_input = safe_input(c("读取超时时间（秒）[默认 30]: ", "yellow"))
    try:
        read_timeout = float(read_timeout_input)
    except Exception:
        read_timeout = 30

    retry_input = safe_input(c("单次请求重试次数 [默认 8]: ", "yellow"))
...
def scrape(proxies, params):
    ...
    try:
        ...
        r, ftype = request_with_retry(session, BASE,
                                       ...
                                       retry_count=5)
        if r and r.status_code == 200:
            # 检查是否被 CF 拦截（HTTP 200 但内容是 CF 验证页）
            body_text = r.text
            cf_markers = ['challenge-platform', 'cf-browser-verification',
                          'Just a moment', 'cf-challenge', '_cf_chl', 'cf-disturbed']
            if any(mk in body_text.lower() for mk in cf_markers):
                print(c(f"✗ HTTP {r.status_code} (Cloudflare 拦截)", "red"))
                print(c(f"  返回内容长度: {len(body_text)} bytes", "dim"))
                raise Exception("Cloudflare 拦截: 你的 IP 被识别为机器人，请配置代理后重试")
            print(c(f"✔ HTTP {r.status_code} (内容 {len(body_text)} bytes)", "green"))
        else:
            status = r.status_code if r else "None"
        ...
        err_msg = str(e)
        print(c(f"\n✗ 连接失败: {err_msg}", "red"))
        if "Cloudflare" in err_msg or "拦截" in err_msg:
            print(c("", "reset"))
            print(c("  ╔══════════════════════════════════════════════════╗", "red"))
            print(c("  ║  ⛔  Cloudflare 拦截！                            ║", "red"))
            print(c("  ║  请在启动脚本时选择代理 (HTTP/SOCKS5)           ║", "yellow"))
            print(c("  ║  或更换网络/VPN 后重试                           ║", "yellow"))
            print(c("  ╚══════════════════════════════════════════════════╝", "red"))
        else:
    ...
    confirm = safe_input(c("\n确认开始爬取? [Y/n]: ", "yellow")).lower()
